...
try {
    ...
    $leaderboardQuery = "
        SELECT 
            ...
            RANK() OVER (ORDER BY COALESCE(d.completed_count, 0) DESC, ur.total_points DESC) as rank_position
        FROM user_rewards ur
        ...
        WHERE u.is_active = 1
        ORDER BY COALESCE(d.completed_count, 0) DESC, ur.total_points DESC
        LIMIT 10
    ";
